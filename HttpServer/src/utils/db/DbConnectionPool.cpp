    ...
    namespace db
    {
        ...
        void DbConnectionPool::checkConnections()
        {
            ...
            while (true)
            {
                ...
                {
                    // --- 第一步：把连接全部从队列里"借出"来 ---
                    // 关键：必须把连接从队列里 pop 出来而不是只 copy，
                    // 否则主线程也能拿到同一个连接对象，导致两个线程同时
                    // 操作同一个 MySQL socket 句柄 -> Segfault / Hang

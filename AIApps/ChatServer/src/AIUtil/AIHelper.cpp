...
{
    // 默认使用阿里云大模型
    strategy = StrategyFactory::instance().create("qwen");
}
...
std::string AIHelper::chat(int userId, std::string userName, std::string sessionId, std::string userQuestion, std::string modelType)
{
    ...
    std::string keyName = modelType;
    if (modelType == "deepseek")
    {
        modelType = "deepseek-chat";
        keyName = "deepseek";
    }
    else if (modelType == "qwen-plus")
        keyName = "qwen";
    else if (modelType == "doubao-pro-32k")
        keyName = "doubao";
    else if (modelType == "deepseek-chat")
        keyName = "deepseek";
    ...
    // ✅ NEW: Inject API Key — config.json first, then env var (via AIConfig::getApiKey which already checks both)
    std::string apiKey = config.getApiKey(keyName);
    if (apiKey.empty())
    {
        apiKey = config.getApiKey(modelType);
    }

    if (apiKey.empty())
    {
        std::cout << "[Warning] No API key found for " << keyName << ", using env or default if available." << std::endl;
    }
    else
    {
    ...
    // 说明支持MCP
    //  config is already loaded at function entry
...
json AIHelper::executeCurl(const json &payload)
{
    ...
    // 强制 HTTP/1.1：关闭 ALPN 协商和 HTTP/2，避免分帧层错误
    curl_easy_setopt(curl, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_1);
    curl_easy_setopt(curl, CURLOPT_SSL_ENABLE_ALPN, 0L); // 禁用 ALPN，防止 TLS 层失误协商 h2
    curl_easy_setopt(curl, CURLOPT_TIMEOUT, 60L);        // 60s 超时
...
void AIHelper::pushMessageToMysql(int userId, const std::string &userName, bool is_user, const std::string &userInput, long long ms, std::string sessionId)
{
    ...
    // Debug log to check quoting
    std::cout << "[AIHelper] Generated SQL: " << sql << std::endl;
